...
class temporalGPR:
    ...
    def initialize_model(self, gp_model, train_period):
        ...
        self.train_x = self.standardized_coords(self.observations\
                            .sel(year=self.trainslice)).to(mps_device)
        self.train_y = torch.from_numpy(self.observations\
                        .sel(year=self.trainslice).values).float().to(mps_device)
        ...
        self.prior_y = torch.from_numpy(self.priors.sel(year=self.trainslice)\
                            .transpose('models','year').values).float().to(mps_device)
        ...
        self.prior_x = self.standardized_coords(self.observations['year']).to(mps_device)
        # Defining the likelihood function. Here we used a likelihood with fixed heteroscedastic noise
        self.likelihood = gpytorch.likelihoods.FixedNoiseGaussianLikelihood(\
                            noise=torch.from_numpy(self.ob_noise.values).float()\
                            .to(mps_device).unsqueeze(dim=0),\
                            learn_additional_noise=True).to(mps_device)
        # Putting it all together. Defining the actual GPR model for gpytorch
        self.model = gp_model(self.train_x, self.train_y,\
                            self.prior_x,self.prior_y,self.prior_ys,self.likelihood).to(mps_device)
    ...
    def fit(self,fitslice):
        ...
        self.fitted_x = self.standardized_coords(self.observations\
                            .sel(year=self.fitslice)).to(mps_device)
        self.fitted_y = torch.from_numpy(self.observations\
                            .sel(year=self.fitslice).values).float().to(mps_device)
    ...
    def predict(self):
        ...
        with torch.no_grad(), gpytorch.settings.fast_pred_var():
            ...
            mean = self.predicted_dist.mean.cpu().numpy()
            std = self.predicted_dist.stddev.cpu().numpy()
            # The predictions are stored as xarray using coordinates from observations
            self.predicted = (self.observations*0+mean)\
                                    .to_dataset(name='mu_star')
            self.predicted['std'] = self.observations*0+std
...
class spacetimeGPR:
    ...
    def initialize_model(self, gp_model, train_period):
        ...
        self.train_y = torch.from_numpy(self.observations.sel(year=self.trainslice)\
                        .stack(spacetime=['year','lon','lat']).values).float().to(mps_device)
        self.prior_y = torch.from_numpy(self.priors.sel(year=self.trainslice)\
        ...
        self.prior_x = self.standardized_coords(self.observations\
                                        .sel(lat=self.lat,lon=self.lon)).to(mps_device)
        self.nsamples = self.train_y.size()[0]
        self.psize = self.prior_ys.size()[0]
        # Defining the likelihood function. Here we used a likelihood with fixed heteroscedastic noise
        self.likelihood = gpytorch.likelihoods.FixedNoiseGaussianLikelihood(\
                            noise=torch.from_numpy(self.ob_noise.values.flatten()).float().to(mps_device)\
                            .repeat(len(self.observations['year'])),\
                            learn_additional_noise=True).to(mps_device)
        # Putting it all together
        self.model = gp_model(self.train_x, self.train_y,self.prior_x,\
                              self.prior_y,self.prior_ys, self.likelihood).to(mps_device)
