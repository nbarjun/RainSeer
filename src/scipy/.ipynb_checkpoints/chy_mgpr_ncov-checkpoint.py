...
class cmip_common_hyparams_mgpr_nc:
    def __init__(self, kernel, observations, smoothed, lon, lat):
        ...
        self.mnoise = (self.observations-self.priors).coarsen(year=noise_time,boundary='trim').mean()\
                        .sel(year=self.noise_years).var('year',ddof=1)
    ...
    def summed_likelihood(self, initial_guess):
        ...
        for oosm_index in range(len(models)):
            ...
            noise_cov = np.cov((self.observations.sel(models=oos_model)\
                        -self.priors.sel(models=oos_model)).sel(year=self.noise_years)\
                        .stack(space=['lon','lat']).transpose('space','year'))
            self.noise_cov.append(noise_cov)
            nyrs = len(oos_gcm.unstack()['year'])
            oosm_noise = np.tile(np.tile(noise_cov,nyrs).T,nyrs)
    ...
    def fit(self, oos_model, tslice):
        ...
        noise_cov = np.cov((self.observations.sel(models=oos_model)\
                        -self.priors.sel(models=oos_model)).sel(year=self.noise_years)\
                        .stack(space=['lon','lat']).transpose('space','year'))
        nyrs = len(self.Y_fit.unstack()['year'])
        self.oosm_noise = np.tile(np.tile(noise_cov,nyrs).T,nyrs)
        ...
        self.K += self.oosm_noise #* np.eye(*self.K.shape)
    ...
    def predict(self,oos_model):
        ...
        self.iv_noise = (self.observations.sel(models=oos_model,lat=self.lat,lon=self.lon)\
                        -self.priors.sel(models=oos_model,lat=self.lat,lon=self.lon))\
                        .sel(year=self.noise_years).var(dim='year',ddof=1)
        ...
        predicted['nstd'] = xr.DataArray((np.diag(cov_f_star)+self.iv_noise.item())**.5,\
                                        coords={'year':self.observations.year})
        return predicted
