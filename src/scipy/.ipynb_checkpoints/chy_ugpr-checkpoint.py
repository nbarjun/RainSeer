...
class cmip_common_hyparams_ugpr:
    def __init__(self, kernel, observations, smoothed):
        self.kernel = kernel
        self.observations = observations
        self.priors = smoothed
        self.ob_noise = (self.observations-self.priors).sel(year=slice(1870,1975))\
                            .var(dim='year',ddof=1)
    ...
    def train(self, tslice):
        ...
        self.X_train = self.observations.sel(year=tslice)['year']
        self.Y_train = self.observations.sel(year=tslice)
        self.p_train = self.priors.sel(year=tslice)
    ...
    def squared_exponential_kernel(self, X, variance=1.0, length_scale=1.0, noise=1e-10):
        X = np.atleast_2d(X).T
        # Compute pairwise squared Euclidean distances
        dists = distance.cdist(X, X, metric='sqeuclidean')  / length_scale
        # Compute the kernel matrix
        K = variance * np.exp(-0.5 * dists ) + noise * np.eye(*dists.shape)
        return K
    ...
    def fit(self, oos_model, tslice):
        ...
        models = self.observations['models'].values
        other_gcms = sorted(list(set(models)-set({oos_model})))
        self.Y_fit = self.observations.sel(models=oos_model,year=self.fitslice).values
        self.X_fit = self.observations['year'].sel(year=self.fitslice).values
        ...
        self.K += oosm_noise * np.eye(*self.Y_fit.shape)
        
    def predict(self,oos_model):
        models = self.observations['models'].values
        other_gcms = sorted(list(set(models)-set({oos_model})))
        X_pred = self.observations['year'].values
        other_gcms_smoothed = self.priors.sel(models=other_gcms)\
                                    .transpose('year','models')
        oosm_noise = self.ob_noise.sel(models=oos_model).item()
        mnoise = self.mnoise.sel(models=oos_model).item()

        self.pm_star = self.prior_mean(other_gcms_smoothed)
        self.pc_star = self.prior_covariance(other_gcms_smoothed)
    ...
    def plot_predicted(self, posterior, oos_model, nchoice, plot_priors=False):
        ...
        models = self.observations['models'].values
        oos_gcm = self.observations.sel(models=oos_model)
        other_gcms = sorted(list(set(models)-set({oos_model})))
        other_gcms_smoothed = self.priors.sel(models=other_gcms).transpose('year','models')
        ...
        self.priors.sel(models=oos_model).plot(ax=ax,c='tab:green',alpha=.85,label=oos_model,lw=.8)
